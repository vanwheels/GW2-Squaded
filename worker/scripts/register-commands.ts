 ...
 *
 * The actual command definitions live in `src/discord/command-catalog.ts`, not here — shared with
 * `discord/commands/help.ts`'s `/help` output so the two can't drift apart. This script is just
 * the "PUT it to Discord" side effect.
 */

import { commands } from '../src/discord/command-catalog'
