...
export function EquipmentEditor({
  ...
  // Food/utility intentionally list the FULL catalog, not a pre-filtered "WvW meta" subset, per
  // explicit user direction (see TODO.md) — this deliberately includes "Feast"/"Tray"/"Pot"/
  // "Station" items, which most WvW squads actually use over individually-carried food/utility
  // since one placed item shares its buff with the group (confirmed by the user 2026-08-06, after
  // an earlier version of this file wrongly excluded them). Their buff numbers are resolved via
  // `Consumable.sharedBuffSource`/`borrowSharedContainerBonuses` in `fetch-gear-upgrades.ts`, not
  // filtered out here.
