...
import { factLine } from './fact-numbers'
import { healingLinesForSkill } from './healing-calc'
import { damageLinesForSkill } from './damage-calc'

function realValueLine(fact: Fact, damageByLabel: Map<string, number>, healingByLabel: Map<string, number>): string | null {
  if (typeof fact.text !== 'string') return null
  if (fact.type === 'Damage' && damageByLabel.has(fact.text)) {
    return `${fact.text}: ${damageByLabel.get(fact.text)!.toLocaleString()}`
  }
  if (fact.type === 'AttributeAdjust' && fact.target === 'Healing' && healingByLabel.has(fact.text)) {
    return `${fact.text}: ${healingByLabel.get(fact.text)!.toLocaleString()}`
  }
  return null
}
